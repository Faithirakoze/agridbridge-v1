...
const ICONS = { maize:'🌽', beans:'🫘', sorghum:'🌾', irish_potato:'🥔', sweet_potato:'🍠', cassava:'🌿', vegetables:'🥬' };
...
export default function CropsPage() {
  ...
  useEffect(() => {
    Promise.all([client.get('/crops'), client.get('/farms')])
      .then(([c, f]) => { setCrops(c.data); setFarms(f.data); })
      .catch(console.warn)
  ...
  async function handleSubmit(e) {
    e.preventDefault();
    if (!farms.length) return setError('Register a farm first via the API.');
            ...
            <div key={crop.id} className="card flex items-center gap-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-gray-400">
                  ...
                  {crop.planted_at ? `Planted ${new Date(crop.planted_at).toLocaleDateString('en-RW', { day:'numeric', month:'short' })}` : ''}
                </p>
              </div>
